...
import DitheringFilter from "../util/filter/DitheringFilter";


const calculateIndexToRowCol = (index: number, width: number) => {
    ...
    useEffect(() => {
        ...
        if (video && filter && context) {
            ...
            const draw = (video: HTMLVideoElement, context: CanvasRenderingContext2D, width: number, height: number) => {
                ...
                applyImageFilter(image, DitheringFilter({threshold}));
